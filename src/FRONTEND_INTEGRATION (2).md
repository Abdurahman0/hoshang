...
### 3.2.1 Login Lock Rule

Backend login himoyasi:

- ketma-ket 5 ta noto‘g‘ri login urinishidan keyin user 2 minutga lock bo‘ladi
- bu qoida barcha rollarga, jumladan `developer` ga ham tegadi
- lock vaqtida `POST /api/auth/login/` `429 Too Many Requests` qaytaradi

Frontend tavsiya:

- `429` bo‘lsa login form ustida vaqtinchalik blok xabari ko‘rsating
- userga 2 minut kutish kerakligini ayting
...
```json
[
  { "id": 1, "value": "new", "label": "New", "sort_order": 1, "is_active": true },
  { "id": 2, "value": "contacted", "label": "Contacted", "sort_order": 2, "is_active": true }
]
```
...
### 4.6.1 Lead Status Management Page

Bu blok faqat `admin` va `developer` uchun.
...
```json
{
  ...
  "sort_order": 7,
  "is_active": true,
  "created_at": "2026-05-18T18:00:00.000000+05:00",
  "updated_at": "2026-05-18T18:00:00.000000+05:00"
}
```

Muhim:

- `Lead.status` API ichida string code sifatida qoladi
- agar status code o‘zgarsa, backend mavjud leadlardagi shu status code ni ham yangilaydi
- agar status leadlarda ishlatilayotgan bo‘lsa, delete `400` qaytaradi
...
### 7.3 Active AI Config Create Or Initialize

Endpoint:

```http
POST /api/ai-settings/active/
```

Bu endpoint frontend uchun server ilk marta bo‘sh chiqqanida ishlatiladi.

Behavior:

- agar active config bo‘lmasa `201 Created`
- agar active config bor bo‘lsa shu active config update bo‘ladi va `200 OK`
